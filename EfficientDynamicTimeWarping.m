function [DistanceVal,indxCol,indxRw] = EfficientDynamicTimeWarping(refSample,testSample)
...
if(N == M) % each set containing same no. of feature
    ...
    if(noOfSamplesInTestSample < (2*noOfSamplesInRefSample)) % then you can apply itekura band
        theta = calSmallItekura(DistMat,noOfSamplesInRefSample,noOfSamplesInTestSample);
    else
    ...
    for n=2:noOfSamplesInRefSample
        for m=2:noOfSamplesInTestSample
            D(n,m)=DistMat(n,m)+min([D(n-1,m-1),D(n,m-1),D(n-1,m)]); % Thanks for the suggestion by "Sven Mensing" in Matlab Central
            if (D(n,m) > theta)
                break;
            end
        end
