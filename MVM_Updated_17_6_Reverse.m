...
function [pathCost,pathTarget,indxcol,indxrow,distSum,jumpcost] = MVM_Updated_17_6_Reverse(refSample,testSample,weight,bal)
...
%     end
    ...
    if(noOfSamplesInTestSample >= noOfSamplesInRefSample)
        ...
        distSum = normalizationTech_0(indxrow,indxcol,distSum,noOfSamplesInRefSample,noOfSamplesInTestSample,smalJC);
...
function distSum = normalizationTech_1(indxrow,indxcol,distSum,~,noOfSamplesInTestSample,jumpcost)
jumpedQueryEle = 0;
jumpedTargetEle = 0;
...
distSum = distSum/(size(indxcol,1))+ (begSkip + endSkip);%+jumpedQueryEle+jumpedTargetEle;
return
end
function distSum = normalizationTech_2(~,indxcol,distSum,~,noOfSamplesInTestSample,jumpcost)
endSkip = abs(noOfSamplesInTestSample - indxcol(end,1));
begSkip = (indxcol(1,1) -1);
distSum = distSum + ((begSkip + endSkip)*jumpcost);
