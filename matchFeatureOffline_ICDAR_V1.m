function resultArr  = matchFeatureOffline_ICDAR_V1(keepAllFeature,myGT,technique,percent,temImgRefBin,temImgRefGrey,refname)
...
for k = 1:nImages
    ...
    if(( (1/beta) <=(nForeGdPixelsRef / nofForeGdPixelsTest)  ) && ( (nForeGdPixelsRef / nofForeGdPixelsTest) < beta  ) )
        level1 {cnt1,1} = testName; level1 {cnt1,2} = k;cnt1 = cnt1 + 1;
        if (( (1/sigma) <=( (realRWRef * realColRef)  / (realColTest * realRwTest) ) ) && ...
                ( ( (realRWRef * realColRef)  / (realColTest * realRwTest) ) < sigma  ) )
            level2 {cnt2,1} = testName;level2 {cnt1,2} = k; cnt2 = cnt2 + 1;
            if ( ((1/alfa) <=( (realColRef/realRWRef)  / (realColTest/realRwTest) )  ) ...
                    && ( ( (realColRef/realRWRef)  / (realColTest/realRwTest) ) < alfa  ) )
                level3 {cnt3,1} = testName;level3 {cnt1,2} = k; cnt3 = cnt3 + 1;
            end
        end
    end
end
% [refinedPrunnedSet] = calculateHitMissCorrect(myGT,level1,keepAllFeature);
% [~,missed1,totalGT] = calculateHitMiss(myGT,refinedPrunnedSet);
% fprintf('The missed elements after level1 is: %d out of %d; whereas total considered prunned elements %d out of total %d\n',missed1,totalGT,length(refinedPrunnedSet),nImages);
%
% [refinedPrunnedSet] = calculateHitMissCorrect(myGT,level2,keepAllFeature);
% [~,missed2,totalGT] = calculateHitMiss(myGT,refinedPrunnedSet);
% fprintf('The missed elements after level2 is: %d out of %d; whereas total considered prunned elements %d out of total %d\n',missed2,totalGT,length(refinedPrunnedSet),nImages);

[refinedPrunnedSet] = calculateHitMissCorrect(myGT,level3,keepAllFeature);
[~,missed3,totalGT] = calculateHitMiss(myGT,refinedPrunnedSet);
fprintf('The missed elements after level3 is: %d out of %d;  whereas total considered prunned elements %d out of total %d\n',missed3,totalGT,length(refinedPrunnedSet),nImages);



keepDistAllTest = zeros(1,1);
keepName = cell(1,1);
for tt = 1:size(refinedPrunnedSet,1)
    k = refinedPrunnedSet{tt,2};
    ...
    keepDistAllTest(tt,:) = distVal;  
    keepName{tt,1} = testName;
end
