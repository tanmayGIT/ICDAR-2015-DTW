% This code abd the MVM_Updated_10 is particularly same but here we have not
% used the weight in jump cost calculation, whereas in MVM_Updated_7 we
% have used weight factor for jump cost calculation.

function [pathCost,pathTarget,indxcol,indxrow,distSum,jumpcost] = MVM_Updated_10(refSample,testSample,weight)
pathTarget = 1;
...
%         statmatx = minArr;
    ...
    if(noOfSamplesInTestSample >= noOfSamplesInRefSample) % this condition k is satisfied only when the above statement
        for ji = 1:1:(noOfSamplesInTestSample)
